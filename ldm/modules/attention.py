        ...
        if not self.gen_pkl:
            ...
            if use_mask:
                ...
                if q_injected is not None and k_injected is not None:
                    ...
                    if use_nstyle_path:
                        ...
                        ### 26/01/01: pi_star 버전
                        pi_star = pi_style_total
                        print(f"2-style은 기존것 그대로 구현중. pi_star={pi_star}")
                        ...
                        # 첫 연산만 out-of-place (안전성 확보)
                        cat_sim = cat_sim - mean
                ...
                # style injection이 일어나지 않는 경우 -- 원본과 동일하게 진행
                    ...
                    print(attn_matrix_scale, '1 attn_matrix_scale 0-5에서 안씀.')
            ...
            # {마스크용 npy파일이 없으면 마스킹 적용 x} or {self.attn2 즉, cross-attention}  -- 원본과 동일하게 진행  
                ...
                if q_injected is not None or k_injected is not None:
                    # print(attn_matrix_scale, '2 attn_matrix_scale 0-5에서 안씀.')
                    sim *= attn_matrix_scale    
                sim *= self.scale
                attn = sim.softmax(dim=-1)
