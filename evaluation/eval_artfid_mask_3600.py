...
def get_limited_stylized_and_style_paths(path_to_stylized, path_to_style, style_pairs):
    stylized_image_paths = limit_paths_by_content(get_image_paths(path_to_stylized, sort=True), style_pairs)
    style_ids = get_style_ids_from_stylized_paths(stylized_image_paths)
    style_image_paths = filter_style_paths_by_style_ids(get_image_paths(path_to_style, sort=True), style_ids)
...
def get_limited_stylized_and_content_paths(path_to_stylized, path_to_content, style_pairs):
    stylized_image_paths = limit_paths_by_content(get_image_paths(path_to_stylized, sort=True), style_pairs)
    content_image_paths = limit_paths_by_content(get_image_paths(path_to_content, sort=True), style_pairs)
...
def get_limited_mask_paths(mask_path, style_pairs):
    return limit_paths_by_content(get_image_paths(mask_path, sort=True), style_pairs)
...
def compute_fid_infinity(path_to_stylized, path_to_style, mask_path, batch_size, device, num_points=15, num_workers=1, back=False, style_pairs=180):#mask넣어주기
    ...
    mask_image_paths = get_limited_mask_paths(mask_path, style_pairs)#mask path가져오기
    stylized_image_paths, style_image_paths = get_limited_stylized_and_style_paths(path_to_stylized,
                                                                                     path_to_style,
                                                                                     style_pairs)
    assert len(stylized_image_paths) == len(mask_image_paths), \
           f'Number of stylized images and number of mask images must be equal.({len(stylized_image_paths)},{len(mask_image_paths)})'
...
def compute_art_fid(path_to_stylized,
    ...
    if mode == 'art_fid_inf':
        fid_value = compute_fid_infinity(path_to_stylized, path_to_style, mask_path, batch_size, device, num_workers, back=back, style_pairs=style_pairs)#mask넣어주기
    elif mode == 'art_fid':
